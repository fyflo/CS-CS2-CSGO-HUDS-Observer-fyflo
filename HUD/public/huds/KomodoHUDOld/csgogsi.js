var CSGOGSI = /** @class */ (function () {
  ...
  CSGOGSI.prototype.digest = function (raw) {
    ...
    var ctOnLeft =
      Object.values(raw.allplayers).filter(function (_a) {
        var observer_slot = _a.observer_slot,
          team = _a.team;
        return (
          observer_slot !== undefined &&
          observer_slot > 1 &&
          observer_slot <= 5 &&
          team === "CT"
        );
      }).length > 2;
    var ctExtension = null,
      tExtension = null;
    if (this.teams[0]) {
    ...
    var teamCT = {
      ...
      matches_won_this_series:
        (ctExtension && ctExtension.map_score) ||
        teams[0].matches_won_this_series,
      side: "CT",
    ...
    var teamT = {
      ...
      matches_won_this_series:
        (tExtension && tExtension.map_score) ||
        teams[1].matches_won_this_series,
      side: "T",
    ...
    // observed = players.filter(function (player) { return player.steamid === raw.player.steamid; })[0] || null;
    var data = {
        ...
        ? {
            ...
            player: bomb
              ? players.filter(function (player) {
                  return player.steamid === bomb.player;
                })[0]
              : undefined,
            site:
              bomb.state === "planted" ||
              ...
              bomb.state === "planting"
                ? this.findSite(
                    raw.map.name,
                    bomb.position.split(", ").map(Number)
                  )
                : undefined,
          }
        : null,
    ...
    Object.keys(players).forEach(function (steamid) {
      ...
      parsed.push(
        _this.parsePlayer(
          players[steamid],
          steamid,
          players[steamid].team === "CT" ? teams[0] : teams[1]
        )
      );
    });
    return parsed;
  ...
  CSGOGSI.prototype.parsePlayer = function (oldPlayer, steamid, team) {
    var extension = this.players.filter(function (player) {
    ...
    var player = {
      ...
      position: oldPlayer.position.split(", ").map(function (pos) {
        return Number(pos);
      }),
      forward: oldPlayer.forward.split(", ").map(function (pos) {
        return Number(pos);
      }),
      team: team,
  ...
  CSGOGSI.prototype.removeListener = function (eventName, listener) {
    ...
    this.listeners.set(
      eventName,
      listOfListeners.filter(function (callback) {
  ...
  CSGOGSI.prototype.findSite = function (mapName, position) {
    var mapReference = {
      de_mirage: function (position) {
        return position[1] < 1500 ? "A" : "B";
      },
      de_cache: function (position) {
        return position[1] > 0 ? "A" : "B";
      },
      de_overpass: function (position) {
        return position[2] > 400 ? "A" : "B";
      },
      de_nuke: function (position) {
        return position[2] > -500 ? "A" : "B";
      },
      de_dust2: function (position) {
        return position[0] > -500 ? "A" : "B";
      },
      de_inferno: function (position) {
        return position[0] > 1400 ? "A" : "B";
      },
      de_vertigo: function (position) {
        return position[1] < 1400 ? "A" : "B";
      },
      de_train: function (position) {
        return position[1] > -450 ? "A" : "B";
      },
      de_ancient: function (position) {
        return position[1] < -500 ? "A" : "B";
      },
      de_anubis: function (position) {
        return position[1] > 0 ? "A" : "B";
      },
    };

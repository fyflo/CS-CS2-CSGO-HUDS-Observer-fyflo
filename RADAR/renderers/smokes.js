...
socket.element.addEventListener("smokes", (event) => {
  ...
  for (let smoke of smokes) {
    ...
    let smokeID =
      "smoke" +
      smoke.id +
      (smoke.position.x + smoke.position.y + smoke.position.z);
    ...
    if (!smokeElement) {
      ...
      smokeElement.style.height = smokeElement.style.width =
    ...
    smokeElement.style.bottom =
      global.positionToPerc(smoke.position, "y") + "%";
...
socket.element.addEventListener("explosion", (event) => {
  ...
  for (let smokeElement of document.getElementById("smokes").childNodes) {
    ...
    let dist =
      (Math.abs(smokeX - pos.x) + Math.abs(smokeY - pos.y)) *
      global.mapData.resolution;
...
socket.element.addEventListener("flashbangs", (event) => {
  ...
  for (let flashbang of flashbangs) {
    ...
    if (!flashbangElement) {
      ...
      flashbangElement.style.left =
        global.positionToPerc(flashbang.position, "x") + "%";
      flashbangElement.style.bottom =
        global.positionToPerc(flashbang.position, "y") + "%";

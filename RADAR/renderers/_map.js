  ...
  fetch(window.location.origin + `/maps/${mapName}/meta.json5`)
    ...
    .then((data) => {
      ...
      document.getElementById(
        "radarBackground"
      ).src = `/maps/${mapName}/radar.png`;

      if (global.config.radar.showLogos) {
        document.getElementById(
          "radarLogos"
        ).src = `/maps/${mapName}/overlay_logos.png`;
      }
      ...
      if (global.config.radar.hideAdvisories) {
        ...
        document.getElementById("advisory").style.left =
          global.mapData.advisoryPosition.x + "%";
        document.getElementById("advisory").style.bottom =
          global.mapData.advisoryPosition.y + "%";

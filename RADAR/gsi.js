...
server.listen(config.game.networkPort, config.game.host, () => {
  console.info(
    `GSI input expected at http://${config.game.host}:${config.game.networkPort}`
  );
});
  ...
  req.on("end", () => {
    ...
    if (game.grenades) {
      ...
      for (let nadeID in game.grenades) {
        ...
        if (nade.type == "flashbang" && parseFloat(nade.lifetime) >= 1.4) {
          ...
          nade.type != "decoy" &&
          nade.velocity != "0.000, 0.000, 0.000" &&
          (nade.type != "smoke" || nade.effecttime == "0.000")
        ) {
          let pos = nade.position.split(", ");
          let owner = game.allplayers[nade.owner];
    ...
    if (game.phase_countdowns) {
      process.send({
        type: "canbuy",
        data: !(
          ["live", "bomb", "defuse", "over"].includes(
            game.phase_countdowns.phase
          ) && parseFloat(game.phase_countdowns.phase_ends_in) < 95
        ),
      });
    }
